...
func do_zones(ch *Character, arguments string) {
	...
	for zone := range ch.Game.Zones.All() {
...
func do_mlist(ch *Character, arguments string) {
	...
	for wch := range ch.Game.Characters.All() {
...
func do_goto(ch *Character, arguments string) {
	...
	if firstArgument == "plane" {
		...
		var found *Plane = nil
		for plane := range ch.Game.Planes.All() {
			if plane.Id == id {
				found = plane
				break
			}
		...
		if ch.Room != nil {
			...
			for character := range destination.Characters.All() {
				if character != ch {
	...
	for gch := range ch.Game.Characters.All() {
